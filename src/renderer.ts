...
const gisbuddy = (
  window as unknown as {
    gisbuddy: {
      ...
      onAgentEvent: (
        listener: (conversationId: string, event: unknown) => void,
      ) => () => void;
...
function rlog(scope: string, msg: string, extra?: unknown): void {
  try {
    gisbuddy.log("info", scope, msg, extra);
  } catch {
...
function rlogErr(scope: string, msg: string, extra?: unknown): void {
  try {
    gisbuddy.log("error", scope, msg, extra);
  } catch {
    /* IPC may not be ready */
  }
...
async function handleAutoTitle(conv: Conversation, proxy: AgentProxy) {
  ...
  try {
    ...
    /* IPC may fail — title stays '新对话', retries next agent_end */
  }
}
...
async function switchToConversation(convId: string) {
  ...
  fileViewData = null;
  if (mapRafHandle) {
    cancelAnimationFrame(mapRafHandle);
    mapRafHandle = 0;
  }
  if (mapInstance) {
    mapInstance.remove();
  ...
  const switched = await gisbuddy.agentSwitch(
    conv.id,
    currentCwd,
  ...
  if (switched.sessionFilePath && switched.sessionFilePath !== conv.sessionId) {
    ...
    try {
      await gisbuddy.setConversationSessionId(
        conv.id,
        switched.sessionFilePath,
      );
    } catch {
      /* IPC may fail — next switch will retry */
    }
  }
...
function renderSidebar() {
  ...
  if (sidebarCollapsed) {
    return html`
      <div
        ...
        style="position:absolute;top:0;left:0;z-index:9999;${isMac ? "height:38px;padding-left:80px;padding-right:16px;" : "height:38px;padding-left:16px;padding-right:140px;"}display:flex;justify-content:flex-start;align-items:center;gap:2px;${DRAG}"
      >
        <button
  ...
  return html`
    <div
      data-testid="sidebar"
      style="width:240px;height:100vh;border-right:1px solid #d8d0c2;display:flex;flex-direction:column;background:#e8e3d8;font-family:system-ui,sans-serif;"
    >
      <!-- Header -->
      <div
        @dblclick=${handleDragDblClick}
        style="height:38px;flex-shrink:0;${isMac ? "padding-left:80px;" : "padding-left:16px;"}padding-right:${isMac ? "16px" : "140px"};display:flex;justify-content:flex-end;align-items:center;gap:2px;${DRAG}"
      >
        <button
          @click=${handleNewConversation}
      ...
      <div style="padding:8px 16px;border-top:1px solid #d8d0c2;">
        <span style="font-size:11px;color:#a09886;"
          >${conversations.length} 个对话</span
        >
      </div>
    </div>
  `;
...
function renderFileTree() {
  ...
  return html`
    <div
      style="width:220px;height:100vh;border-left:1px solid #d8d0c2;display:flex;flex-direction:column;background:#e8e3d8;font-family:system-ui,sans-serif;font-size:13px;"
    >
      <div
        @dblclick=${handleDragDblClick}
      ...
      <div style="flex:1;overflow-y:auto;padding:4px 0;">
        ${
          upOne
            ? html`
                <div
                  @click=${() => handleDirClick(parentDir(currentDir))}
        ...
        ${fileTreeEntries.map(
          (entry) => html`
            <div
              @click=${() => (entry.isDirectory ? handleDirClick(entry.path) : handleFileClick(entry.path))}
            ...
            >
              <span
                >${entry.isDirectory ? "📁" : FILE_ICONS[entry.ext] || "📄"}</span
              >
              <span
                style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                >${entry.name}</span
...
function renderFileView() {
  if (!activeFilePath) {
    ...
    const inner =
      currentConvId && chatPanel
        ? chatPanel
        : html`<div
            style="display:flex;align-items:center;justify-content:center;height:100%;color:#7a7468;"
          >
            选择一个对话
          </div>`;
    ...
    return html`
      <div style="display:flex;flex-direction:column;height:100%;">
        <div
          @dblclick=${handleDragDblClick}
          style="position:relative;height:38px;flex-shrink:0;display:flex;align-items:center;padding:0 16px;border-bottom:1px solid #d8d0c2;background:#ece8de;font-size:13px;color:#5a544a;font-family:system-ui,sans-serif;${sidebarCollapsed ? "" : DRAG}"
        >
          ${isMac ? html`<span style="width:${sidebarCollapsed ? "140px" : "64px"};flex-shrink:0;"></span>` : ""}
          <span
            style="position:absolute;left:0;right:0;text-align:center;pointer-events:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:0 ${isMac ? "16px" : "150px"} 0 ${isMac ? "16px" : "16px"};"
            >${titleText}</span
          >
        </div>
  ...
  const header = html`
    <div
      @dblclick=${handleDragDblClick}
      style="display:flex;align-items:center;gap:8px;padding:8px 16px;border-bottom:1px solid #d8d0c2;background:#ece8de;${DRAG}"
    >
      <button
        @click=${handleCloseFile}
        style="border:none;background:none;cursor:pointer;font-size:14px;color:#7a7468;${NO_DRAG}"
      >
        ← 返回
      </button>
  ...
  if (!fileViewData) {
    return html`${header}
      <div
        style="display:flex;align-items:center;justify-content:center;flex:1;color:#7a7468;"
      >
        加载中...
      </div>`;
  }
  ...
  switch (data.type) {
    case "text":
      body = html`<pre
        style="flex:1;overflow:auto;margin:0;padding:16px;background:#e2ddd0;font-family:monospace;font-size:13px;white-space:pre-wrap;"
      >
${data.content}</pre>`;
      break;
    case "image":
      body = html`<div
        style="flex:1;overflow:auto;display:flex;align-items:center;justify-content:center;background:#d6d0c2;"
      >
        <img
          src="${data.content}"
          alt="${data.name}"
    ...
    default:
      body = html`<div
        style="flex:1;display:flex;align-items:center;justify-content:center;color:#a09886;font-size:14px;"
      >
        ${data.message || "无法预览此文件"}
      </div>`;
      break;
  }
  ...
  render(
    html`
      ...
      >
        ${renderSidebar()}
        <div
          style="flex:1;height:100vh;display:flex;flex-direction:column;min-width:0;"
        >
          ${renderFileView()}
        </div>
        ${renderFileTree()}
...
function promptApiKey(container: HTMLElement): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    form.style.cssText =
      "display:flex;flex-direction:column;gap:12px;width:360px;max-width:90vw;padding:24px;background:#ece8de;border-radius:8px;box-shadow:0 4px 24px rgba(0,0,0,0.15);font-family:sans-serif;";
    ...
    input.required = true;
    input.style.cssText =
      "padding:8px 10px;border:1px solid #d8d0c2;border-radius:4px;font-size:14px;outline:none;background:#e8e3d8;color:#5a544a;";
    ...
    cancelBtn.style.cssText =
      "padding:6px 12px;border:1px solid #d8d0c2;background:#e8e3d8;color:#5a544a;border-radius:4px;cursor:pointer;font-size:14px;";
    ...
    submitBtn.style.cssText =
      "padding:6px 12px;border:none;background:#6b7d5e;color:#fff;border-radius:4px;cursor:pointer;font-size:14px;";
    ...
    overlay.style.cssText =
      "position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.4);z-index:9999;";
...
async function initApp() {
  ...
  render(
    html`<div
      style="display:flex;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;color:#7a7468;"
    >
      GISBuddy loading...
    </div>`,
    app,
