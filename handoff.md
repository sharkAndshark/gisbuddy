...
- **测试**: Vitest (单元/集成 77 个) + Playwright (E2E 10 个, 3 real-API skip)
- **包管理**: npm
...
npm run test:e2e     # build + playwright (10 tests + 3 skipped real-API)
npm run test:all     # 全量 (87 tests)
npm run lint         # eslint
...
tests/
  ...
  integration/agent-flow.test.ts # AgentSession + IPC 主路径 (6 tests)
...
| IndexedDB SessionsStore 仍在 setupAppStorage | 低 | 暂留兼容；未来清理（持久化已迁到 main 的 SessionManager JSONL） |
| bundled GDAL PATH 注入未做 | 中 | dev 用系统 GDAL；release 包需 `defineTool` 覆盖 bash 注入 gdal-bin/（Phase 3 后的 P2） |

## 未完成工作（优先级排序）

### P1 — bundled GDAL PATH 注入
- 当前: dev 环境用系统 GDAL（brew 装在 /opt/homebrew/bin），release 包 `gdal-bin/` 为空
- 需要: 用 `defineTool` 覆盖 bash 注入 `gdal-bin/` 到 PATH（复用 `electron/gdal-path.ts`）
