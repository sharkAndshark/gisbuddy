...
| Tool IPC | B43 | tool-exec 未知工具名返回错误 | `electron/main.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | mock 未知 toolName |

## File Read — 文件内容分析

| 模块 | 编号 | 可观测行为 | 来源文件 | 测试编号 | 测试质量(1-5) | 测试合理性(1-5) | 解耦建议/豁免 | 备注 |
|------|------|-----------|---------|---------|:------------:|:--------------:|:------------:|------|
| File Read | B44 | read-file 处理文本文件 | `electron/main.ts` | — | — | 5 | 可解耦 / ⚠ 应测 | mock fs |
| File Read | B45 | read-file 对 .json 文件自动 pretty-print | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
| File Read | B46 | read-file 返回 GeoJSON map 类型（兼容 CRS） | `electron/main.ts` | — | — | 5 | 可解耦 / ⚠ 应测 | — |
| File Read | B47 | read-file GeoJSON 不兼容 CRS 时降级为 text | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
| File Read | B48 | read-file 返回 image base64 data URI | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
| File Read | B49 | read-file 解析 Shapefile (.shp) 为 GeoJSON | `electron/main.ts` | — | — | 5 | 可解耦 / ⚠ 应测 | — |
| File Read | B50 | read-file Shapefile 不兼容 CRS 时拒绝显示 | `electron/main.ts` | — | — | 5 | 可解耦 / ⚠ 应测 | — |
| File Read | B51 | read-file Shapefile .dbf 缺失时仍解析 | `electron/main.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | — |
| File Read | B52 | read-file Shapefile 解析异常时返回 error | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
| File Read | B53 | read-file 超限检查：text>512KB, image>10MB, geojson>50MB, shp>500MB | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
| File Read | B54 | read-file 不支持的文件类型返回错误提示 | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
| File Read | B55 | read-file 读取异常时返回统一错误格式 | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |

## List Directory — 目录浏览

| 模块 | 编号 | 可观测行为 | 来源文件 | 测试编号 | 测试质量(1-5) | 测试合理性(1-5) | 解耦建议/豁免 | 备注 |
|------|------|-----------|---------|---------|:------------:|:--------------:|:------------:|------|
| List Dir | B56 | list-directory 过滤 . 开头的隐藏文件 | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
| List Dir | B57 | list-directory 目录排在文件前面 | `electron/main.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | — |
| List Dir | B58 | list-directory 同名按字母排序 | `electron/main.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | — |
| List Dir | B59 | list-directory 返回 name/path/isDirectory/size/ext | `electron/main.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | — |
...
| Renderer | B63 | AppStorage 初始化：IndexedDB backend + ProviderKeysStore + SessionsStore | `src/renderer.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | mock IndexedDB |
| Renderer | B64 | switchToConversation: 中止旧 Agent、创建新 Agent、设置 ChatPanel | `src/renderer.ts` | E01 | 4 | 3 | 可解耦 / ⚠ 应测 | E2E 已有部分覆盖 |
| Renderer | B65 | switchToConversation: restore 已保存的 messages (SessionsStore.loadSession) | `src/renderer.ts` | — | — | 5 | 可解耦 / ⚠ 应测 | mock SessionsStore |
| Renderer | B66 | switchToConversation: 新对话自动生成 sessionId 并持久化 | `src/renderer.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | 验证 setConversationSessionId 调用 |
| Renderer | B67 | agent_end: 自动保存 session 到 SessionsStore | `src/renderer.ts` | — | — | 5 | 可解耦 / ⚠ 应测 | mock SessionsStore.saveSession |
| Renderer | B68 | agent_end: 首次回复后自动生成标题（截取前 30 字符） | `src/renderer.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | 验证 renameConversation 调用 |
| Renderer | B69 | msgListFix: message_end 时强制 message-list 更新（Lit array 引用问题） | `src/renderer.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | mock Agent + DOM |
...
| Renderer | B73 | 侧边栏删除对话按钮（✕） | `src/renderer.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | E2E 可测 |
| Renderer | B74 | deleteConversation: 同时清理 IndexedDB session 数据 | `src/renderer.ts` | — | — | 4 | 可解耦 / ⚠ 应测 | mock SessionsStore.deleteSession |
| Renderer | B75 | switchSeq 竞态保护：stale 调用在 await 后返回 | `src/renderer.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | 验证快速切换不会挂错 agent |
...
| 模块 | 编号 | 可观测行为 | 来源文件 | 测试编号 | 测试质量(1-5) | 测试合理性(1-5) | 解耦建议/豁免 | 备注 |
|------|------|-----------|---------|---------|:------------:|:--------------:|:------------:|------|
| Electron | B80 | configure IPC: 保存 API Key 并设置到 AppStorage | `electron/main.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | mock IPC |
| Electron | B81 | create-project IPC: 打开文件夹选择对话框 | `electron/main.ts` | — | — | 2 | ✓ 认可 | 依赖 Electron dialog |
| Electron | B82 | create-project 取消时返回 null | `electron/main.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | mock dialog 取消 |
| Electron | B83 | get-projects / get-conversations 等 CRUD IPC 注册 | `electron/main.ts` | — | — | 3 | 可解耦 / ⚠ 应测 | 验证 handler 调用 CM 方法 |
...
| I01-I09 | `tests/integration/tools.test.ts` | 9 |

> **统计**: 总共 86 个行为，已测 41+8=49 个（单元 41 + E2E 8），豁免 19 个（合理性 < 3 或依赖 Electron API），应测未测 18 个。
